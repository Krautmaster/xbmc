...
bool CDVDPlayer::OpenAudioStream(int iStream, int source)
{
  ...
  || m_CurrentAudio.hint != hint)
  {
    if(m_CurrentAudio.id >= 0)
    {
      CLog::Log(LOGDEBUG, " - audio codec's hints have changed, must close previous stream");
      CloseAudioStream(true);
    }
...
bool CDVDPlayer::OpenVideoStream(int iStream, int source)
{
  ...
  || m_CurrentVideo.hint != hint)
  {
    if(m_CurrentVideo.id >= 0)
    {
      CLog::Log(LOGDEBUG, " - video codec's hints have changed, must close previous stream");
