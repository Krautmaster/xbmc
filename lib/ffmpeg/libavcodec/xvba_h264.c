                       ...
                       av_unused uint32_t       size)
{
  ...
  assert(render);
  pic_descriptor = render->picture_descriptor_buffer->bufferXVBA;
...
static int end_frame(AVCodecContext *avctx)
{
  ...
  pic_descriptor = render->picture_descriptor_buffer->bufferXVBA;
  iq_matrix = render->iq_matrix_buffer->bufferXVBA;

  av_dlog(avctx, "end_frame()\n");

  /* Fill in Picture Parameters*/
  pic_descriptor->profile                                     = avctx->profile;
  ...
  pic_descriptor->chroma_format                               = s->chroma_format;
                        ...
                        uint32_t        size)
{
    ...
    bitstream_buffer = render->data_buffer->bufferXVBA;
    data_control = render->data_control;

    av_dlog(avctx, "decode_slice(): buffer %p, size %d\n", buffer, size);

    memcpy((uint8_t*)bitstream_buffer+render->data_buffer->data_size_in_buffer, buffer, size);
    render->data_buffer->data_size_in_buffer += size;

    data_control = av_fast_realloc(
         data_control,
         &render->data_control_size,
         sizeof(unsigned int)*(render->num_slices + 1)
    );
    data_control[render->num_slices] = size;
    render->num_slices++;
